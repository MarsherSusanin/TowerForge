...
- Expose only the AI Designer allowlist. Build/package commands, raw file access, shell, web tools, plugins, skills, and arbitrary executable/path inputs are unavailable.
...
- Users can choose ChatGPT/Claude account billing or the existing direct API-key providers without configuring external MCP.
