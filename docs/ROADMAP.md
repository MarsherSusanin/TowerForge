...
- **Tier 0 — grid/autotile and renderer conformance (shipped).** Maps independently select hex/odd-r or square/cardinal topology. Canvas, Phaser, and Studio Playtest share geometry, exact mouse/keyboard tile actions, Wang/autotile resolution, weighted deterministic variants, transforms, and square/hex sector composition. CI builds the `4 templates × 2 grids × 2 renderers` matrix.
- **Tier 3 — themed tile packs (shipped).** Verdant Frontier and Frostbound Citadel bundle generated original backgrounds, coordinated palettes, square edge-16 sheets, and hex edge-64 sheets. Studio previews before apply; CLI/MCP use revision guards, confined copies, backups, validation, and rollback.
- **Tier 3 — deterministic custom scripting (shipped).** TowerScript files under `scripts/` bind event-driven rules to global, mission, map, wave, tower, enemy, ability, and terrain contexts. V2 adds tile entry/change events and budgeted runtime terrain mutation. The same runtime serves Studio/headless/Canvas/Phaser and remains isolated from host APIs.
...
- **Asset and narrative pipeline (partly shipped).** Standalone/atlas-frame sprites, PNG + Tiled TSJ/TSX Wang import, deterministic variants, map/grid tile bindings, safe project-local import, audio, narrative, and two generated-original square/hex themed tile packs are validated through Studio/build/MCP. _Still open:_ complete tower/enemy sprite families, batch binding, and opt-in generation hooks.
