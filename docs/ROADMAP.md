...
- **Tier 1 next — live AI co-designer (shipped).** Studio **AI Designer** tab: a chat panel that runs the **author → simulate → diagnose → patch → re-simulate** loop through Anthropic, OpenAI, or OpenRouter. Provider keys are stored separately in browser `localStorage` (never committed), model IDs are extensible, and OpenRouter exposes a live tool-capable catalog. `packages/studio/server.mjs` `/api/ai/chat` runs a zero-dependency agentic loop over the shared MCP `callTool` surface (`packages/mcp/tools.mjs`), with `projectDir` forced to the server's project, a fetch timeout + client-disconnect abort, and step cap. It streams tool calls/results as NDJSON; after validated write tools such as `apply_validated_patch`, `set_enemy_stat`, `upsert_tower`, `add_wave_group`, or `bind_sprite`, the editor reloads from disk. External MCP agents can still drive the same tool surface.
