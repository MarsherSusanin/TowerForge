...
export const DEFAULT_TERRAIN_TYPES: Record<string, TerrainTypeDefinition> = Object.freeze({
...
export function normalizeTerrainTypes(
  authored: Record<string, Partial<TerrainTypeDefinition>> | undefined,
  waterGroundSpeedFactor = DEFAULT_TERRAIN_TYPES.water!.groundSpeedMultiplier
): Record<string, TerrainTypeDefinition> {
  const defaults = Object.fromEntries(
    Object.entries(DEFAULT_TERRAIN_TYPES).map(([id, value]) => [
      id,
  ...
  for (const [id, value] of Object.entries(authored ?? {})) {
    ...
    defaults[id] = {
      ...
      tags: Array.isArray(value.tags) ? [...new Set(value.tags.filter((tag): tag is string => typeof tag === "string"))] : [...fallback.tags]
    };
  }
  return defaults;
}
