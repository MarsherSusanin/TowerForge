export const TOWERFORGE_AGENT_GUIDE_VERSION = 5;
...
Workflow:
1. In a workspace-bound plugin session, call list_workspace_projects first and select_workspace_project when more than one project is available. Never supply or request an absolute projectDir.
2. Read compact state with get_project_summary and list/get tools. Do not guess ids or current values.
3. Call describe_schema with the relevant domain and use recipes before creating new shapes.
4. Preview supported writes, pass current revision tokens, and prefer granular tools over broad patches.
5. Run validate_project after writes. For tile work, run preview_tile_binding, then render_tileset_preview and inspect its PNG contact sheet for seams or fallback placeholders; for behavior use playtest_report, and run balance_report for balance-affecting changes.
6. Keep changes small, explain conflicts or validation failures, and finish with the evidence that verifies the result.
