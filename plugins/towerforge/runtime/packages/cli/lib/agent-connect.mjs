...
//
// Design notes:
// - Codex and Claude Code can install TowerForge as a plugin. Direct MCP entries remain available
//   for clients without marketplace support and for explicit single-project integrations.
...
export function agentClientConfigs(projectDir, serverPath, nodePath = process.execPath) {
  ...
  return [
    ...
    {
      id: "codex-plugin",
      label: "Codex (marketplace plugin)",
      scope: "user",
      file: "plugins/towerforge (Git marketplace)",
      how: "Preferred Codex integration. Installs the TowerForge skill and a workspace-bound bundled MCP runtime; no project path or API key is stored in Codex config.",
      snippet: [
...
// The marketplace plugin is the preferred Codex path. connectCodexUser() remains for existing
// direct MCP installations and explicit single-project setups. It installs an MCP config block and
// prompt only after a user-requested `--write`; Studio never calls it implicitly.
